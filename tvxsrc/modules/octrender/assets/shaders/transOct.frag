...
void voxelDwordGet(out uint voxel, uvec3 pos) {
	uint mortonIdx;
	pos *= 256;
	morton32(mortonIdx, pos.x, pos.y, pos.z);
	mortonVoxelDword(voxel, mortonIdx);
}
bool vdGetHasChild(uint voxel) { return bool(voxel & 0xFFu); }
...
int getvoxel(vec3 pos, float size) {
	uint voxel;
	voxelDwordGet(voxel, uvec3(pos));
	if (vdGetHasChild(voxel)) {
		return 1;
	} else if (vdGetIsFilled(voxel)) {
		return 2;
	} else {
		return 0;
	}
